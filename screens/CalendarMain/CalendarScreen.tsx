...
import { makeAuthorisedRequest, SuccessfulResponseType, isSuccessfulResponseType } from 'utils/makeAuthorisedRequest';
...
import { getDateStringFromDateObject, getDateStringsBetween } from 'utils/datesAndTimes';
...
import { TasksReducerActionType } from 'reduxStore/slices/tasks/types';
...
const parseFixedTaskResponse = (res: FixedTaskResponseType): FixedTaskParsedType => {
  return {
    ...res,
...
const parseFlexibleTaskResponse = (res: FlexibleTaskResponseType): FlexibleTaskParsedType => {
  return {
    ...res,
...
function CalendarScreen({jwtAccessToken, allTasks, setAllTasksProp}: CalendarProps) {
  ...
  const getAllTasks = (): void => {
    ...
    makeAuthorisedRequest<TaskResponseType[]>(jwtAccessToken, `http://${vuetApiUrl}/core/task/`).then(res => {
  ...
  const formatAndSetTasksPerDate = (): void => {
    ...
    for (const task of allTasks) {
      if (isFixedTaskResponseType(task)) {
        ...
        const parsedTask: FlexibleTaskParsedType = parseFlexibleTaskResponse(task)
  ...
  const dayCalendars = Object.keys(tasksPerDate).map((date) => (
    <DayCalendar date={tasksPerDate[date].dateObj} key={date} tasks={tasksPerDate[date].tasks} />
  ));
...
const mapDispatchToProps = (dispatch: Dispatch<TasksReducerActionType>) => {

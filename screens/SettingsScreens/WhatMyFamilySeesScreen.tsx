...
const CategoryToggle = ({ category, checked, onChange }: CategoryToggleProps) => {
  return <TransparentView style={{
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'flex-end',
    marginBottom: 10
  }}>
    <Text style={{ marginRight: 10, width: 100 }}>{category.readable_name}</Text>
    <Checkbox
      checked={checked}
...
export default function WhatMyFamilySeesScreen() {
  ...
  const {
    data: familyViewPermissions,
    isLoading: isLoadingPermissions
  } = useGetAllFamilyCategoryViewPermissionsQuery()

  const [createFamilyCategoryViewPermission, createRes] = useCreateFamilyCategoryViewPermissionMutation()
  const [deleteFamilyCategoryViewPermission, deleteRes] = useDeleteFamilyCategoryViewPermissionMutation()
  ...
  for (const id of allCategories.ids) {
    const permissionObj = Object.values(familyViewPermissions.byId).find(perm => perm.category === id)
    if (permissionObj) {

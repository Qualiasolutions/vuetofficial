...
import { useCreatePreferredDaysMutation, useGetAllPreferredDaysQuery, useUpdatePreferredDaysMutation } from "reduxStore/services/api/settings"
...
const EditPreferredDaysModal = ({ categoryId, visible, onRequestClose }: EditTaskLimitModalProps) => {
  const {
    data: preferredDays,
    isLoading: isLoadingPreferredDays
  ...
  const [preferencesToUpdate, setPreferencesToUpdate] = useState<number | null>(null)

  useEffect(() => {
    if (preferredDays) {
      const preferencesToEdit = Object.values(preferredDays.byId).find(conf => (
  ...
  return <Modal visible={visible} onRequestClose={onRequestClose} style={{ width: '100%' }}>
    <TransparentView style={{ flexDirection: 'row', flexWrap: 'wrap', width: '80%', marginBottom: 20 }}>
      {
        DAYS.map(day => <TransparentView style={{ marginHorizontal: 10 }}>
...
export default function PreferredDayPreferencesScreen() {
  ...
  const { data: allPreferredDays, isLoading: isLoadingPreferredDays } = useGetAllPreferredDaysQuery();

  const { t } = useTranslation()

  const [categoryToEdit, setCategoryToEdit] = useState<null | number>(null)

  const isLoading = isLoadingCategories || isLoadingPreferredDays || !allCategories || !allPreferredDays
  ...
  const PreferencesPressable = ({ categoryId, text }: { categoryId: number; text: string; }) => {
    return <Pressable
      onPress={() => setCategoryToEdit(categoryId)}
    >
  ...
  for (const id of categoryIds) {
    const categoryPreferredDays = Object.values(allPreferredDays.byId)
      .find(preferences => (preferences.category === id))

    if (categoryPreferredDays) {
      preferenceCells[id] = <PreferencesPressable
        categoryId={id}
        text={DAYS.filter(day => categoryPreferredDays[day]).map(day => capitalize(day).slice(0, 2)).join(", ")}
      />
    } else {
      preferenceCells[id] = <PreferencesPressable
        categoryId={id}

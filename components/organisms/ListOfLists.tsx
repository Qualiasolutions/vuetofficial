...
export default function ListOfLists({ entityId }: { entityId: number }) {
  ...
  const listEntities = childEntities.filter(id => allEntities.byId[id].resourcetype === "List")

  const existingLists = listEntities.length > 0 ? <EntityListPage
    entityTypes={["List"]}
    entityTypeName={"List"}
    entityFilters={[(ent: EntityResponseType) => ent.parent === entityId]}
  /> : null

  return <WhiteFullPageScrollView contentContainerStyle={styles.container}>

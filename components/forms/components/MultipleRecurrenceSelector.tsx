...
export default function MultipleRecurrenceSelector({ value, reverse, firstOccurrence, disabled, onChange, max }: Props) {
  ...
  const existingRecurrences = recurrences.map((recurrence, i) => <RecurrenceSelector
    key={i}
    value={recurrence}
    reverse={reverse}
    ...
    onChange={(newRecurrence) => {
      ...
      if (newRecurrence) {
        ...
        onChange((value || []).filter(oldRecurrence => (oldRecurrence !== recurrence)).filter(r => r))
      }
    }}
  />)

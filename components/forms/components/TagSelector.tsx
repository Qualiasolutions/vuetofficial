...
export default function TagSelector({ value, onChange }: Props) {
  ...
  const { data: userDetails, isLoading: isLoadingUserDetails } = getUserFullDetails()
  ...
  const preparedData = {
    entities: allEntities ? Object.values(allEntities.byId).map((entity: EntityResponseType) => ({
      ...entity,
    ...
    >
      {
        (value.entities.length > 0)
          ? value.entities.map(entityId => <Text key={entityId}>{allEntities.byId[entityId].name}</Text>)
    ...
    <ListingModal
      ...
      onSelect={(selectedEntity) => {
        if (value.entities.includes(selectedEntity.id)) {
          const newEntities = value.entities.filter(id => id !== selectedEntity.id)
          onChange({

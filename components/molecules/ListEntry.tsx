import { CustomFile, ImagePicker, PickedFile, SmallImagePicker, WhiteImagePicker } from 'components/forms/components/ImagePicker';
import React, { useEffect, useRef, useState } from 'react';
import { Text, View, StyleSheet, Pressable, Image, TextInput, Animated, TouchableWithoutFeedback, Touchable, TouchableOpacity } from 'react-native';
...
export default function ListEntry({ children, key, listEntry }: {children: React.ReactNode, key: string, listEntry: ListEntryResponse}) {
    ...
    const [formUpdateListEntry, formUpdateListEntryResult]= useFormUpdateListEntryMutation();
    ...
    const [phoneNumber, updatePhoneNumber] = useState<string>(listEntry.phone_number);
    const [phoneNumberErrorMessage, setPhoneNumberErrorMessage] = useState<string>('');
    const [showDeleteAndArchive, setShowDeleteAndArchive] = useState<boolean>(false);
    ...
    const updateNoteInDb = (note: string) => {
        const response = updateListEntry({
            id: listEntry.id,
            notes: note,
        })
    }
    ...
    const updatePhoneNumberInDb = (number: string) => {
        ...
        .catch((error) => setPhoneNumberErrorMessage('Please enter a valid phone number (starting with +44)'))
    }
  ...
  return (
    <Animated.View style={{width: '100%', display: 'flex', flexDirection: 'row', minHeight: 100, transform: [{ translateX: listEntryTranslateX }]}}>
    <GestureRecognizer 
        onSwipeLeft={(state) => 
            {
        ...
        >
         {showIcons ? 
            ...
            : <></>}
    <Pressable onPress={() => {updateShowIcons(!showIcons)}}>
        <View key={key} style={styles.content}>  
            ...
            {listEntry.image ? <Image source={{uri: listEntry.image}} style={styles.image} /> : <></> }
            {listEntry.notes || addingNote ? 
                <TextInput style={styles.input} placeholder="Enter a new note" onChangeText={(note) => updateNote(note)} defaultValue={listEntry.notes} /> : <></> }
            {listEntry.phone_number || addingPhoneNumber ?
            <>
                {phoneNumberErrorMessage !== '' ? <Text style={styles.errorMessage}>{phoneNumberErrorMessage}</Text> : <></> }
                <TextInput style={styles.input} placeholder="Enter a phonenumber" onChangeText={(num) => updatePhoneNumber(num) } defaultValue={listEntry.phone_number} /> 
            </>
                : <></>
        }
        </View>
        <Autosave experimentData={note} saveDataToDb={updateNoteInDb} />
        <Autosave experimentData={phoneNumber} saveDataToDb={updatePhoneNumberInDb} />
    </Pressable>
    </GestureRecognizer>

    <TouchableHighlight style={{ width: 35, display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 1}} onPress={()=> {
        deleteListEntry(listEntry.id)
        }}>
            <View style={{backgroundColor: "#AC3201", flex: 1, justifyContent: 'center', alignItems: 'center', width: 35}}>
